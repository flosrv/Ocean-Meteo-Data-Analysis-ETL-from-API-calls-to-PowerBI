...
from database import engine_staging, cleaned_marine_data, get_db_staging
from sqlalchemy import func, asc
from api.models import MarineNumericFields, MarineNonNumericFields  # Assurez-vous que cette importation est correcte
...
# Route pour obtenir la médiane d'une colonne choisie dans cleaned_marine_data
@router.get("/median")
async def get_median(
    column: MarineNumericFields,
    ...
    if column_name not in cleaned_marine_data.c.keys():
        raise HTTPException(status_code=400, detail=f"Colonne '{column_name}' non trouvée.")

    column_obj = cleaned_marine_data.c[column_name]

    # Construire la requête avec un filtre par Station ID si fourni
    query = select(column_obj).where(cleaned_marine_data.c[column_name] != None)
    
    if station_id:
        query = query.where(cleaned_marine_data.c["Station ID"] == station_id)
    ...
    min_date = db.execute(select(func.min(cleaned_marine_data.c["Datetime"]))).scalar()
    max_date = db.execute(select(func.max(cleaned_marine_data.c["Datetime"]))).scalar()
...
async def get_moving_median_7_days(
    ...
    column: MarineNumericFields = Query(..., description="Colonne numérique pour la médiane mobile"),
    station_id: str = Query(None, description="ID de la station à filtrer"),
    db: Session = Depends(get_db_staging)
):
    """
    ...
    # Construire la requête avec un filtre par Station ID si fourni
    query = select(column_obj).where(cleaned_marine_data.c["Datetime"] >= start_date)
    query = query.where(cleaned_marine_data.c["Datetime"] <= end_date)
...
async def get_records_in_range(
    ...
    if greater_than is not None:
        filters.append(cleaned_marine_data.c[column.value] > greater_than)
    
    if less_than is not None:
        filters.append(cleaned_marine_data.c[column.value] < less_than)

    if station_id:
        filters.append(cleaned_marine_data.c[MarineNonNumericFields.station_id.value] == station_id)

    if start_date:
        try:
            start = datetime.strptime(start_date, "%Y-%m-%d")
            filters.append(cleaned_marine_data.c[MarineNonNumericFields.datetime.value] >= start)
        except ValueError:
    ...
    if end_date:
        try:
            ...
            filters.append(cleaned_marine_data.c[MarineNonNumericFields.datetime.value] <= end)
        except ValueError:
            return {"error": "end_date invalide (attendu: YYYY-MM-DD)"}

    query = db.execute(
        select(cleaned_marine_data).where(and_(*filters))
    )

    results = []
    for row in query.fetchall():
        record = dict(row._mapping)
        dt_raw = record.get(MarineNonNumericFields.datetime.value)
        if isinstance(dt_raw, datetime):
            record[MarineNonNumericFields.datetime.value] = dt_raw.strftime("%Y-%m-%d %H:%M")
