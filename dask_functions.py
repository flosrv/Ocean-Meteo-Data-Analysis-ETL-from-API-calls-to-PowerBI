...
def convert_df_columns(df):
    ...
    Args:
    - df: dd.DataFrame. Le DataFrame à traiter.
    
    Returns:
    - dd.DataFrame: Le DataFrame avec les types de données convertis.
    """
    ...
    for col in df.columns:
        ...
        if df[col].dtype == 'object' and 'date' in col.lower():
            try:
                df[col] = dd.to_datetime(df[col], errors='raise')
            except ValueError:
                pass
        
        # Convertir les booléens (is_day) en int
        if col == "is_day":
            df[col] = dd.to_numeric(df[col], errors='coerce').fillna(0).astype(int)

    # Assurer les types numériques pour les colonnes déjà numériques mais mal typées
    for col in df.select_dtypes(include=['float64', 'int64']).columns:
        df[col] = df[col].astype(dd.Float64Dtype())  # Garantir une gestion correcte des NaN dans les colonnes numériques
...
def fetch_table_data(engine, table_name: str) -> dd.DataFrame:
    """
    Récupère les données d'une table MySQL dans une DataFrame Dask.
    ...
    Returns:
        dd.DataFrame: DataFrame contenant les données de la table.
    """
    try:
        query = text(f"SELECT * FROM `{table_name}`;")
        with engine.connect() as connection:
            # Utiliser Dask pour lire directement la table en SQL
            df = dd.read_sql(query, connection)
        ...
        # Retourner un DataFrame Dask vide en cas d'erreur
        return dd.from_delayed([])  # Un DataFrame Dask vide
    
def create_table_in_mysql(df: dd.DataFrame, table_name: str, engine):
...
def insert_new_rows(engine, df: dd.DataFrame, table_name: str):
    """
    ...
    try:
        ...
        df["Datetime"] = dd.to_datetime(df["Datetime"])  # S'assurer que la colonne est bien en datetime
...
def handle_null_values_dask(df):
    ...
    for column in df.columns:
        null_percentage = missing_percent[column].compute()  # Calcul du pourcentage de valeurs manquantes
        ...
        if null_percentage > threshold:  # Si + de X% de valeurs manquantes (dépend du tag)
            ...
            if pd.api.types.is_numeric_dtype(df[column]):
                median_value = df[column].median().compute()
                ...
                mode_value = df[column].mode().compute()[0]  # Imputation par le mode pour les colonnes non numériques
...
def meteo_api_request(coordinates, mode='historical', days=92, interval='hourly'):
    ...
    Retourne :
        dd.DataFrame : un DataFrame avec les données météo
    """
    ...
    if mode == 'historical':
        ...
        if interval == 'hourly':
            ...
            for i, var in enumerate(hourly_variables):
                hourly_data[var] = dd.from_pandas(pd.Series([round(value, 2) for value in hourly.Variables(i).ValuesAsNumpy()]), npartitions=1)

            return dd.from_pandas(pd.DataFrame(hourly_data), npartitions=1)
            ...
            for i, var in enumerate(daily_variables):
                daily_data[var] = dd.from_pandas(pd.Series([round(value, 2) for value in daily.Variables(i).ValuesAsNumpy()]), npartitions=1)

            return dd.from_pandas(pd.DataFrame(daily_data), npartitions=1)
...
def process_datetime_column(df, column):
    ...
    try:
        df[column] = dd.to_datetime(df[column], errors='coerce', utc=True)
...
def clean_dataframe(df, cols_to_convert, verbose=False):
    ...
    if "water_level_above_mean" in df.columns:
        df["water_level_above_mean"] = df["water_level_above_mean"].fillna(0)

    if "air_temperature" in df.columns:
        df["air_temperature"] = df["air_temperature"].fillna(df["air_temperature"].median())

    if "water_temperature" in df.columns:
        df["water_temperature"] = df["water_temperature"].fillna(df["water_temperature"].median())
...
def convert_columns_to_numeric(df, cols_to_convert):
    ...
    for col in cols_to_convert:
        if col in df.columns:
            try:
                ...
                df[col] = dd.to_numeric(df[col], errors="raise")  # 'raise' lève une erreur si la conversion échoue
            except ValueError:
    ...
    if "is_day" in df.columns:
        try:
            df["is_day"] = dd.to_numeric(df["is_day"], errors="raise").astype(int)
        except ValueError:
...
def rename_columns(df, columns):
    # Assurez-vous que l'entrée est bien un DataFrame Dask ou Pandas
    ...
    if isinstance(columns, list):
        for rename_dict in columns:
            ...
            if existing_columns:
                ...
                # Pour un DataFrame Dask, utiliser `rename` de manière paresseuse
...
def calculate_closer_probabilities(df, columns):
    ...
    # Calculer la probabilité de chaque colonne d'être la plus proche
    probabilities = {}
    for col in columns:
        # La probabilité pourrait être inversement liée à la somme des distances (plus la somme est faible, plus la probabilité est élevée)
        total_distance = 0
        for dist_column in distances:
            if col in dist_column:
                total_distance += df[dist_column].mean()  # Utiliser la moyenne des distances comme une mesure de proximité
        
        # Calculer la probabilité (inversement proportionnelle à la distance totale)
        probabilities[col] = 1 / (total_distance + 1e-5)  # Ajouter un petit epsilon pour éviter la division par zéro
    
    # Normaliser les probabilités pour qu'elles somme à 1
    total_prob = sum(probabilities.values())
    for col in probabilities:
        probabilities[col] /= total_prob
    
    return probabilities, df

def add_hour_day_period_and_month(df, datetime_column='Datetime', overwrite_existing=False):
    """
    ...
    overwrite_existing (bool): Si True, réécrit les colonnes existantes. Par défaut, False.
    ...
    # Vérification des valeurs NaT après conversion
    if df[datetime_column].isnull().any():
        print(f"🚨 Attention : Certaines valeurs dans la colonne '{datetime_column}' sont invalides (NaT).")
    
    # Si overwrite_existing est activé ou si la colonne n'existe pas, ajoutez/écrivez les nouvelles colonnes
    if overwrite_existing or 'DayPeriod' not in df.columns:
        ...
        print("📅 'DayPeriod' ajouté ou mis à jour.")
    
    if overwrite_existing or 'Month' not in df.columns:
        df['Month'] = df[datetime_column].dt.month
        print("📅 'Month' ajouté ou mis à jour.")
    
    if overwrite_existing or 'Hour' not in df.columns:
        df['Hour'] = df[datetime_column].dt.hour
        print("📅 'Hour' ajouté ou mis à jour.")
    
    return df

def create_table_in_mysql_dask(df: dd.DataFrame, table_name: str, engine):
    # Vérifier si la table existe déjà
    check_table_sql = f"SHOW TABLES LIKE '{table_name}';"
    ...
    try:
        with engine.connect() as connection:
            ...
            if result.fetchone():
                ...
                # Définition des colonnes
                column_definitions = []
                # Traitement de chaque partition Dask pour obtenir les colonnes
                sample_df = df.compute()  # Obtenir un échantillon des données en Pandas
...
def insert_new_rows_dask(df: dd.DataFrame, engine, table: str, ref_column: str):
    """
    Insère les nouvelles lignes dans la table MySQL après avoir vérifié si les IDs uniques existent déjà.
    Adaptée pour un DataFrame Dask.

    Args:
    - df (dd.DataFrame): Le DataFrame Dask contenant les nouvelles données.
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            if row_count == 0:
                # La table est vide, insérer toutes les données
                print("⚡ La table est vide. Insertion de toutes les données avec suivi...")
                total = len(df)
                inserted = 0
                # Convertir Dask DataFrame en Pandas et insérer ligne par ligne
                for _, row in df.compute().iterrows():
                ...
                # Filtrer en utilisant Dask
                new_rows_df = df[~df[ref_column].isin(existing_ids)]

                if not new_rows_df.compute().empty:
                    total = len(new_rows_df)
                    print(f"🚀 Insertion de {total} nouvelles lignes...")
                    inserted = 0
                    # Convertir les nouvelles lignes en Pandas et les insérer ligne par ligne
                    for _, row in new_rows_df.compute().iterrows():
                        row_df = pd.DataFrame([row])
                        row_df.to_sql(table, con=engine, if_exists='append', index=False)
                        inserted += 1
                        sys.stdout.write(f"\r⏳ Insertion en cours : {inserted}/{total}")
...
def create_unique_id_dask(df, columns):
    """
    Crée un identifiant unique basé sur des colonnes spécifiées dans un DataFrame Dask,
    avec une gestion spéciale pour les colonnes de type datetime.

    Args:
    df (dask.dataframe.DataFrame): DataFrame Dask sur lequel opérer.
    columns (list): Liste des colonnes à utiliser pour créer l'ID unique.

    Returns:
    dask.dataframe.DataFrame: DataFrame Dask avec un identifiant unique créé et ordonné.
    """
    ...
    def process_datetime_column(col):
        ...
        if pd.api.types.is_datetime64_any_dtype(df[col]):
            return df[col].dt.strftime('%Y%m%d%H%M%S')
    ...
    df['Unique ID'] = dd.concat(unique_columns, axis=1).agg(''.join, axis=1)
...
def display_row_values_dask(df, columns=None):
    ...
    column_widths = [df[col].astype(str).map(len).max().compute() if col in df.columns else 0 for col in columns]
    ...
    for i in range(min(10, len(df))):  # Affiche jusqu'à 10 lignes ou le nombre de lignes disponibles
        row_values = [str(df.iloc[i][col]).ljust(column_widths[j]) for j, col in enumerate(columns)]
